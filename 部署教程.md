...
2. 点击命名空间，创建新的命名空间
   ...
   <img src="./assets/nacos/创建命名空间.png" style="zoom:50%;" />

3. 修改`./data/nacos_config/PPENG_GROUP`中的配置文件，补充所有有`TODO`标识的内容

   例如：elasticSearch.yaml文件
...
4. 将`PPENG_GROUP`文件夹压缩，点击nacos控制台中的`配置管理`,进入`配置列表`，选择`PPeng`命名空间，然后 导入配置
   ...
   <img src="./assets/nacos/导入配置.png" style="zoom:50%;" />
...
推荐将项目保存在`/home/ppeng`文件夹中，具体目录结构如下：

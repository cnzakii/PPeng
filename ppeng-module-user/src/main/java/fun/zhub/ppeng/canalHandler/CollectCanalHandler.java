 ...
 **/
@Component
@CanalTable("t_collect")
@Slf4j
public class CollectCanalHandler extends AbstractCanalHandler<Collect> {
    ...
    public void insert(Collect data) {
        String key = USER_COLLECT_KEY + data.getUserId();
        // 查看是否有-1元素，有则清除
        ...
        log.info("刷新用户({})收藏菜谱缓存", data.getUserId());
     ...
     */
    @Override
    public void update(Collect oldData, Collect newData) {
     ...
     */
    @Override
    public void delete(Collect data) {
        String key = USER_COLLECT_KEY + data.getUserId();
